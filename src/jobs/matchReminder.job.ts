...
export const startMatchReminderJob = () => {
  ...
  cron.schedule("* * * * *", async () => {
    ...
    const oneHourFromNowEnd = new Date(oneHourFromNowStart.getTime() + 59 * 1000); // Up to 59 seconds into the next hour
    ...
    for (const match of upcomingMatches) {
      console.log(`Found upcoming match: "${match.title}" starting at ${match.startTime}`);
      ...
      for (const rsvp of rsvps) {
        ...
        console.log(`Sent push notification to user ${user.id} for match ${match.id}`);
        ...
        if (user.receiveEmailNotifications) {
          ...
          console.log(`Enqueued email for user ${user.email} for match ${match.id}`);
        }
      }
    }
